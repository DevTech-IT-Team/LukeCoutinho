...
function PillarIcon({ type }) {
  ...
  return <span className="lc-six-pillars__icon-svg">{icons[type]}</span>;
}
...
export default function SixPillarsLC() {
  ...
  useEffect(() => {
    const tick = () => {
      ...
      setActiveIndex((i) => (i + 1) % HOME_LC_PILLARS.length);
    };
    const id = window.setInterval(tick, AUTO_INTERVAL_MS);
    return () => window.clearInterval(id);
  }, []);

  const focusPillar = (index) => {
  ...
  return (
    <section className="lc-six-pillars" aria-labelledby="home-six-pillars-title">
      <div className="lc-six-pillars__inner">
        ...
        <div className="lc-six-pillars__rule" aria-hidden="true">
          <div
            className="lc-six-pillars__rule-fill"
            style={{ width: `${((activeIndex + 1) / HOME_LC_PILLARS.length) * 100}%` }}
          />
        </div>

        <div className="lc-six-pillars__scroll">
          <ul
            className="lc-six-pillars__columns"
            role="list"
            aria-label="The six pillars of healing"
          >
            {HOME_LC_PILLARS.map((pillar, index) => {
              const isActive = activeIndex === index;
              return (
                <li key={pillar.id}>
                  <article
                    className={`lc-six-pillars__col${isActive ? ' is-active' : ''}`}
                    style={{
                      '--pillar-color': pillar.color,
                      '--pillar-glow': pillar.glow,
                    }}
                    onMouseEnter={() => focusPillar(index)}
                    onMouseLeave={releasePillar}
                    onFocus={() => focusPillar(index)}
                    onBlur={releasePillar}
                    tabIndex={0}
                    aria-current={isActive ? 'true' : undefined}
                  >
                    <div className="lc-six-pillars__col-top">
                      <div className="lc-six-pillars__orb-wrap">
                        <div className="lc-six-pillars__orb">
                          <PillarIcon type={pillar.icon} />
                        </div>
                      </div>
                    </div>
                    <div className="lc-six-pillars__col-text">
                      <h3>{pillar.title}</h3>
                      <p className="lc-six-pillars__hook">{pillar.tagline}</p>
                    </div>
                  </article>
                </li>
              );
